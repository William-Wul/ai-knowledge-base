...
如果你已经看过我们那篇 [Claude Code 保姆级教程](/stage-5/claude-code-guide)，会发现这篇的结构很像：因为 Codex 和 Claude Code 是同一类东西（都是"能替你动手干活的 AI"），上手的逻辑也相通。两篇可以对照着看。
...
两个高度相似，学会一个另一个很快上手。建议**先挑一个上手就行**（哪个能用上用哪个），把基本节奏跑顺。等工作里真用出依赖、有余力了，再装另一个换着用——它们各有所长，但一开始没必要给自己加负担。可以对照我们的 [Claude Code 保姆级教程](/stage-5/claude-code-guide) 一起看。
...
- **学有余力，再添一把武器**：把 Codex 用顺之后，可以再看 [Claude Code 保姆级教程](/stage-5/claude-code-guide)，两个主流 Agent 工具都掌握，AI 干活这条路就基本打通了
