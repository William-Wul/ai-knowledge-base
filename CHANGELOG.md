...
- 适配 AIHOT 新版综合榜表格，同步机构数、评测数、评测证据、人民币价格与共识指数；校验行数、连续名次、重复模型和关键字段。
- 抓取或解析失败时保留原数据并立即让同步任务报错，取消两天内静默成功的处理。
- 榜单页面按浏览器当前时间检查，超过两天未同步时显示历史数据提醒和原榜入口；修正旧的美元及完整度说明。
- 不修改 GitHub Actions 配置；发布后沿用已有每日任务。
...
- 新增三类案例入口、分类筛选、搜索和独立详情页；首批精选 150 个 Goodcase 案例（图片 75、视频 45、网站与工具 30）。
- 详情提供效果展示、中文讲解、完整原文与中文复制、笔记下载、作者来源。
- 从 1,206 条来源详情中按完整度、可复用性与题材分布筛选；语言不作为筛选条件。补译缺失中文，保留原文并校验 SHA-256。
- 案例快照随站构建，封面本地保存；视频按需从来源加载。移除文案分类，合并网页与工具为“AI 做网站与工具”。
- 首页增加案例入口，左侧导航使用AI 生成图片、AI 创作视频、AI 做网站与工具三分类；详情复用文章布局、面包屑和本页目录。
- 全局载入案例样式，修复生产页面样式缺失；卡片使用本地静态缩略图，移动端单列、适配深色模式。
- 案例 Markdown 由同一份数据生成，进入全站搜索；增加中文分词，修复中文关键词无法命中。
- 返回列表保留分类与搜索位置，视频支持分镜复制，工具组件支持一次复制中文指令和源码。
- 列表每次展示 12 条，仅读取摘要索引；完整提示词拆分到独立详情页面，避免一次加载全部案例。
- 将“来源标注的工具”统一改为“参考工具”，删除读者页中的未复现声明；内部保留来源与验证状态。
- 没有新增依赖或修改发布配置；完成构建、150 条资料校验及整站预览验收。

---

## v1.66 · 2026-09-22

### 进阶实践新文：《只输出判断的模型"Jev" 玩法指南》

- 新增 `docs/stage-2/jev-play-guide.md`：面向零基础读者的 Jev 上手文——主线是「只会涂答题卡的机器」：三种答题方式（选择题 Choice / 打分题 Score / 判断题 Noul）、怎么把工作拆成判断题（四句口诀）、四个玩法（批量过筛子 / 几道题一次问完 / 按把握大小分开处理 / 大模型干活 + Jev 验收）、什么时候别用（含自动驾驶默认倾向、自动交易翻车两个边界案例）
- 配图 10 张实拍 + 4 处 HTML 图示；配图经哈希对源重拷，去掉看不懂的「格式/工具调用报错率」柱状图；文案按 william 反馈两轮平实化（「都不是它的位置」→「都不适合用它」等）
- `stagesData.js` PRACTICE_LINKS 追加 `/stage-2/jev-play-guide`
- 素材：A251 / A252 / A254 / A258 / A259 + 主题笔记 system-one-decision-ai
...
- 新增 `docs/stage-2/learn-prompt-from-openai-anthropic.md`：从两份被公开的系统提示词（OpenAI Astra 5051 行 / Claude Opus 5 2049 行）+ 两家官方提示词指南里，提炼六条普通用户能直接抄的写法——正面说法代替否定式 / 角色一句话、重点写协作方式（两种人设模板）/ 批准放在最后一步（成品报批）/ 写清停止条件（三句可抄句式）/ 规则冲突听谁的（规矩三问自查 + 当场要求优先兜底句）/ AI 八股词黑名单（中文版可抄）；每课按"厂商原文 → 大白话转译 → 可抄写法"展开
- 配图 12 处：5 张真实截图（`docs/public/images/stage-2/2026-09-15/`，CL4R1T4S 仓库两份文件、Astra 权限段落、官方主动性提示词、官方 slop words 黑名单，逐张核查实物、图注按图片实际内容写、图源标一手出处）+ 7 张 HTML 图示卡（手册风格对照、正反对照、人设模板、派活对照、停止条件句式、规矩三问、六条自查清单）
- `stagesData.js` PRACTICE_LINKS 插入新文（位于 writing-no-ai-flavor 之后）
- 素材：A246 / A247（cxuan 系统提示词解读）、A244（官方指南六法则）；外部核验：OpenAI 官方博客 rethinking-skills-and-prompts-for-gpt-6-astra（真实可访问）、Anthropic 提示词工程文档（黄金法则、正面指导原则）
- 写作反馈：william 删去开头"最值得抄的作业"一句（铺垫腔），文章直接从两份文件公开切入
